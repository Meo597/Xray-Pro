...
install_startup_service_file() {
    ...
    if [ ! -f '/etc/init.d/xray' ]; then
        ...
        if ! curl -f -L -o "${TMP_DIRECTORY}init.d/xray" https://github.com/Meo597/Xray-Pro/raw/Xray-install/release/alpinelinux/init.d/xray -sS; then
...
information() {
    ...
    if [ "$CONFDIR" -eq '1' ]; then
        ...
        echo 'installed: /usr/local/etc/xray/07_stats.json'
        echo 'installed: /usr/local/etc/xray/08_fakedns.json'
