...
- **Rebase 增强分支**：本地增强分支，前缀为 `ENHANCE_BRANCH_PREFIX`（例如 `pro`），会被 rebase 到 `STABLE_BRANCH` 上，确保与上游基线保持同步。这些分支遵循严格的命名规范，使用数字前缀以强制合并顺序并防止潜在冲突（例如 `Xray-core/pro/0001/feat-xxxxxxxx` 或 `Xray-core/pro/0001/fix-xxxxxxxxxxx`）。
